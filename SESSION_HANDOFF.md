...
- 2026-06-18 intent: Finish P1 custom mobile sheet alignment from UI/CSS review. Risk: medium because these overlays wrap upgrade, delete, and flashcard generation actions.
- 2026-06-18 result: Updated `UpgradeModal`, `FileListPanel` delete confirmation, and `FlashcardsTab` deck modal to mirror the shared mobile sheet presentation: no mobile edge gap, bottom anchored with `mt-auto`, desktop centered with `sm:my-auto`, safe-area-aware max height, and token sheet shadow. Added an accessible close label to the flashcard dialog close button. No action handlers or data flows changed. Rollback hint: restore the previous wrapper/dialog class strings in those three components.
- 2026-06-18 intent: Finish low-risk token consistency cleanup from UI/CSS review. Risk: medium if replacing every legacy shadow class at once because quiz/mindmap/PDF visual hierarchy could change.
- 2026-06-18 result: Replaced only the high-signal arbitrary shadow classes in the desktop sidebar and PDF toolbar with `shadow-card`. Broader `shadow-sm`/`shadow-md`/`shadow-lg` usage remains as a P2 legacy shadow audit that should be done screen-by-screen with visual comparison. Rollback hint: restore the two previous arbitrary shadow class strings in `App.tsx` and `components/PdfViewer.tsx`.
