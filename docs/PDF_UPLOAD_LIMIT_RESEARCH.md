...
Selected hard-rejection copy:

> 페이지의 내용이 이미지로 구성된 PDF 파일은 페이지 이미지를 읽어야 해서 OCR 처리량이 큽니다. 감지값: 80페이지, 35.0MB. 현재 안정 처리 기준은 50페이지 이하, 50.0MB 이하입니다. 같은 페이지 수라도 스캔 해상도, 표/그림 밀도, 판형, 파일 용량에 따라 처리 시간이 크게 달라질 수 있어요. 단원/장 단위로 나눠서 다시 업로드해 주세요.
