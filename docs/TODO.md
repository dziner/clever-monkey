...
- [x] 2026-06-17 리팩토링 5차: Gemini payload diagnostics helper 분리
  ...
  - 수정: `services/geminiPayload.ts`에 `GeminiPayload`, `summarizeGeminiPayload`, `modelForPayload`, `storagePathForPayload`를 분리.
  - 테스트: `tests/unit/geminiPayload.test.ts` 추가. generateContent, inline OCR, storage OCR, TTS payload의 diagnostic summary를 고정.
  - 주의: prompt, provider routing, OCR extraction, TTS chunking, API endpoint 호출 방식은 변경하지 않음.
  - 검증: `git diff --check`, `npx tsc --noEmit`, targeted Gemini payload unit test, `npm test`, `npm run build`, `npx playwright test` 통과.
