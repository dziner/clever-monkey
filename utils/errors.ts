...
const friendlyError = (raw: string): string => {
    ...
    if (/image-based PDFs over 50MB|scanned\/image PDFs up to 50MB|OCR processing/i.test(raw)) {
        return '페이지의 내용이 이미지로 구성된 PDF 파일은 OCR 처리량이 커서 현재 50MB 이하만 안정적으로 처리할 수 있어요. 파일을 나누거나 압축한 뒤 다시 업로드해 주세요.';
    }
    if (/image-based PDFs over 1,000 pages|over 1,000 pages/i.test(raw)) {
        return '페이지의 내용이 이미지로 구성된 PDF 파일은 페이지 이미지 분석이 필요해 처리량이 큽니다. 파일을 단원/장 단위로 나눠서 다시 업로드해 주세요.';
    }
    return raw;
};
